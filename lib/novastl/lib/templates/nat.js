...
function Nat(options) {
    ...
    var instanceCount = publicAvailabilityZones.length;

    var elasticIps = [];
    for (var i = 0; i < instanceCount; i++) {
        var eip = this._addResource(novaform.ec2.EIP(mkname('Eip' + i), {
            Domain: 'vpc',
        }));
        elasticIps.push(eip);
    }
    ...
    var launchConfiguration = this._addResource(novaform.asg.LaunchConfiguration(mkname('LaunchConfig'), {
        ...
        UserData: novaform.base64(novaform.loadUserDataFromFile(__dirname + '/nat-user-data.sh', {
            ...
            ElasticIps: novaform.join(',', elasticIps.map(function(eip) {
                return novaform.ref(eip.name);
            })),
        }))
    }));
    ...
    var autoScalingGroup = this._addResource(novaform.asg.AutoScalingGroup(name, {
        ...
        MinSize: 1,
        MaxSize: instanceCount + 1, // 1 more for rolling update,
