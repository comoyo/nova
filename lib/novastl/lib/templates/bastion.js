...
function Bastion(options) {
    ...
    var cloudConfig = options.cloudConfig || '';
    ...
    if (cloudConfig) {
        multipartContent.push({
            ...
            mime: 'text/cloud-config',
            encoding: 'utf8',
            filename: 'cloudConfig',
        });
    }

    var multipartData = multipart.generate(multipartContent, {
        from: 'nobody XX XXX XX XX:XX:XX XXXX XXXX',
        boundary: '3f4aec2c-5233-11e5-9d64-e745058d08ed',
    });
    var parts = multipartData.split(placeholder);
    var userdata = (cloudConfig)
        ? novaform.join('', [parts[0], shellscript, parts[1], cloudConfig, parts[2]])
        : novaform.join('', [parts[0], shellscript, parts[1]]);
