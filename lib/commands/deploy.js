...
Command.options = [
    ...
    ['', 'timeout=ARG', 'Time in seconds to wait for completion from AWS'],
];
...
Command.prototype.execute = function() {
    ...
    return q().then(function() {
        ...
        return q.all(stackInfoPromises).then(function(stackInfos) {
            ...
            var dependencyObject = utils.zipObject(componentNames, outputs);
            return _.extend(deploymentConfig, {
        ...
        stack.add(_.map(deploymentConfig.buildResult.parameters, 'param'));
        ...
        if (that.commandOptions.wait) {
            ...
            return that._waitForStack({
                ...
                stackName: deploymentConfig.stackName,
                maxWaitSeconds: that.commandOptions.timeout,
            }, function(status) {
